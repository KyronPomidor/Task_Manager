...
# 29-11-2025 
**Backend Architecture Update**: A major backend restructuring was completed to enable further product development and multi-user support. The previous implementation mixed Domain entities with API and Data layers, returned Domain models directly from controllers, and used the same entities for persistence in the database. This made refactoring and extending the system nearly impossible.
To address these issues, the backend was migrated toward a Clean Architecture structure:
- **Separated Domain, Data, and API** layers according to Clean Architecture principles. 
- **Introduced dedicated Database Entities and Mappers**: removing Entity Framework Core's dependency on Domain models. 
- **API now returns DTOs instead of Domain Entities**: ensuring proper encapsulation and clear data flow. 
- **Added new entities**: required for the upcoming Budget Tracker expansion (API integration pending). 
- Began resolving compatibility adjustments with existing project code. 
- Backend changes now allow proper multi-user support; the previous limitation that forced the frontend to use a hardcoded user ID is nearly eliminated.
This foundational update does not introduce visible user-facing features, but it is critical for stability and future functionality.  Contributors: [Mihail. P](https://github.com/varsarius)
