...
public class TaskAssignToCategoryUseCase
{
    ...
    public async Task ExecuteAsync(Guid taskId, Guid targetCategoryId)
    {
        var task = await _taskRepository.GetByIdAsync(taskId) ?? throw new KeyNotFoundException($"Task with Id '{taskId}' not found.");
        var targetCategory = await _categoryRepository.GetByIdAsync(targetCategoryId) ?? throw new KeyNotFoundException($"Category with Id '{targetCategoryId}' not found.");
