...
public class AskAiChatHandler : IRequestHandler<AskAiChatRequest, string>
{
    ...
    public async Task<string> Handle(AskAiChatRequest request, CancellationToken cancellationToken)
    {
        ...
        {
            sb.AppendLine($"- {task.Title} | Description: {task.Description?.ToString()} | Priority: {task.Priority?.ToString() ?? "none"} | Deadline: {task.Deadline?.ToString("yyyy-MM-dd") ?? "none"} | Is completed?: {task.IsCompleted}");
        }
